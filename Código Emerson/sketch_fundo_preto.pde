...
void draw() {
      ...
      for (int i = jan*(-1); i <= jan; i++) {
        for (int j = jan*(-1); j <= jan; j++) {
          ...
          if (disy >= 0 && disy < aux.height &&
            ...
            float r = green(aux.pixels[pos_aux]);
            media += r;
            qtde++;
          }
        }
  ...
  for (int y = 0; y < aux2.height; y++) {
    for (int x = 0; x < aux2.width; x++) {
      ...
      if (green(aux2.pixels[pos]) > 128 && y < 550) aux2.pixels[pos] = color(255);
  ...
  for (int y=0; y < img.height; y++) {
    for (int x=0; x < img.width; x++) {
      ...
      aux3.pixels[pos] = aux2.pixels[pos];

      if ((aux3.pixels[pos]) == color(255)   ) {
        aux3.pixels[pos] = img.pixels[pos];
      }
    }
  }
  image(aux3, 0, 0);
